...
- [ ] `inventory/models.py`: `OutboundNote(BaseNote)`:
  ...
  - [ ] `destination` CharField(100) blank — nơi nhận khi issue_for_use
  - [ ] `Meta(BaseNote.Meta)`: `db_table="outbound_note"`, verbose "Phiếu xuất", index `to_warehouse`
...
- [ ] `inventory/serializers.py`: `OutboundNoteSerializer` (nested lines, `note_type_label`, `status_label`, `total_quantity` = số dòng, validate: destination/to_warehouse theo `note_type`, replace-all update)
...
- [ ] `inventory/serializers.py`: `SimpleOutboundNoteSerializer` (`{id, number}`) cho `StockMovementSerializer`
...
- [ ] `inventory/filters.py`: `OutboundNoteFilter` — note_type, status, warehouse, to_warehouse, date_from, date_to
...
- [ ] `transfer` thiếu `toWarehouseId` → 400; `toWarehouseId == warehouseId` → 400; `issue_for_use` thiếu `destination` → 400
- [ ] PUT/DELETE phiếu đã chốt → 400; `/void/` thiếu reason → 400
- [ ] Permission: kế toán tạo phiếu → 403, xem → 200
- [ ] Filter note_type / status / warehouse / to_warehouse / date

## Seed

- [ ] `inventory/management/commands/seed_outbound_notes.py`: 1 phiếu xuất cấp + 1 phiếu điều chuyển (tự chốt) — xác nhận tồn 2 kho đúng
