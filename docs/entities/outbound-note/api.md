...
| `POST`   | `/api/outbound-notes/`            | Tạo phiếu **nháp** + dòng          | `{"noteType": "issue_for_use", "date": "2026-08-18", "warehouseId": 1, "destination": "...", "lines": [...]}` | `201` (status=`draft`)      |
...
```json
{
    ...
    "date": "2026-08-18",
    "warehouseId": 1,
    "destination": "Công trường cầu Rạch Giá",
...
| Rule                  | Mô tả                                                    | HTTP |
| --------------------- | -------------------------------------------------------- | ---- |
| Destination required  | `noteType=issue_for_use` mà thiếu `destination`          | 400  |
| Destination forbidden | `noteType=transfer` mà có `destination`                  | 400  |
