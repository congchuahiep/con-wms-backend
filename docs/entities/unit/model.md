...
| 4   | `created_at` | DateTimeField     | auto_now_add |                             |
| 5   | `updated_at` | DateTimeField     | auto_now     |                             |

### 1.2 Field mới — `conversion_type`

| #   | Field             | Kiểu          | Ràng buộc                 | Ghi chú                      |
| --- | ----------------- | ------------- | ------------------------- | ---------------------------- |
| 6   | `conversion_type` | CharField(10) | choices, default="global" | `"global"` hoặc `"material"` |
...
```
Query 1: UnitConversion.objects.filter(from_unit=obj)   ← direct
Query 2: UnitConversion.objects.filter(to_unit=obj, material__isnull=True) ← reverse (chỉ global)
```
